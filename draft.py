...
class CommonMappingFunc:
    ...
    def processMultLine(self, lineNum, processLineNum = 2, currentList = None):
      ...
      for addLineNum in range(1, processLineNum + 1):
        newProcessString = newProcessString + currentList[lineNum + addLineNum]
      return [lineNum + processLineNum, newProcessString]
...
class Show_policymap_interface(CommonMappingFunc):
    ...
    def matchPriLevel(self, inputLine):
      matchResult = re.search('priority level (\d+)',inputLine)
      if matchResult:
        return [True, matchResult.group(1)]
      else:
        return [False, None]     
    ...
    def matchPoliceExceeded(self, inputLine):
    ...
    def matchMulPoliceExceededAction(self, inputLine):
    ...
    def formatOutput(self, notMatchInclude = True):
      ...
      self.matchSingleFunList = [self.matchClassMap, self.matchSerPolicy, self.matchPacketByte, self.matchOfferedDropRate, \
        ...
        self.matchShapeCirBcBe, self.matchTargetShapRate, self.matchQueStats, self.matchBandWidth, self.matchWREDExpWeight, \
        self.matchWREDMeanQueDep, self.matchWREDstart, self.matchWREDPre, self.matchPolice, self.matchPoliceCir , self.matchPoliceConformSpeed]
      self.formatedOutput = []
      skipLine = 0
...
inst = Show_policymap_interface(a)   
inst.formatOutput()     
for i in inst.formatedOutput:
  print(type(i))
for i in inst.formatedOutput:
  print(i)
empList = []
...
inst = Show_policymap_interface(string) 
